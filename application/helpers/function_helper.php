    ...
    if (isset($id) && !empty($id) && isset($discount) && !empty($discount)) {
        ...
        for ($i = 0; $i < count($pid); $i++) {
            for ($j = 0; $j < count($product['product'][$i]['variants']); $j++) {
                ...
                array_push($product['product'][$i]['variants'][$j]['sale_price'], $replace_price);
            }
        }
        return $product;
    } else {
