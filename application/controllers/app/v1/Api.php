                ...
                for ($i = 0; $i < count($tmp_cart_user_data); $i++) {
                    ...
                    if (!empty($product_data[0]['product_id'])) {
                        ...
                        if (!empty($pro_details['product'])) {
                            //flash_sale
                            $id =   $product_data[0]['product_id'];
                            $sale_dis = exists_in_flash_sale($id);
                            if (!empty($sale_dis)) {
                                for ($j = 0; $j < count((array)$sale_dis); $j++) {
                                    for ($k = 0; $k < count((array)$pro_details['product'][$i]['variants']); $k++) {
                                        $sale_amt = get_flash_sale_price($pro_details['product'][$i]['variants'][$k]['price'], $sale_dis[$j]['discount']);
                                        $pro_details['product'][$i]['variants'][$k]['special_price'] = $sale_amt;
                                    }
                                }
                            }
                            //end
