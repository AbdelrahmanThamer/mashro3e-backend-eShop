    ...
    public function place_order($data)
    {
        ...
        if (!empty($product_variant)) {
            ...
            for ($i = 0; $i < count($product_variant); $i++) {
                ...
                // $id =  $product_variant[$i]['varient_product_id'];
                // $sale_dis = exists_in_flash_sale($id);
                // if (!empty($sale_dis)) {
                //     for ($j = 0; $j < count($sale_dis); $j++) {
                //         $sale_amt = get_flash_sale_price($product_variant[$i]['price'], $sale_dis[$j]['discount']);
                //         $product_variant[$i]['special_price'] = $sale_amt;
                //     }
                // }
        ...
        foreach ($user_details as $row) {
            if (!empty($row['items'])) {
                ...
                if (!$this->ion_auth->is_delivery_boy()) {
                    ...
                    if (trim($row['active_status']) != 'awaiting' && $row['is_local_pickup'] == 0) {
